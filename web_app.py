...
st.markdown("""
    ...
    .data-label { color: #8E8E93; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 5px; }
...
# --- 3. MARKET VERIFICATION ENGINE ---

def fetch_market_valuation(card_name, grade_filter=""):
    """
    Forced 'Sold' search using the last_sold_date filter.
    Returns: (Avg Price, Data Count, Verification Data)
    """
    ...
    try:
        ...
        # 90-day window for strict sold data
        lookback_date = (datetime.now() - timedelta(days=90)).strftime('%Y-%m-%dT%H:%M:%S.000Z')
        
        search_query = f"{card_name} {grade_filter}"
        query_encoded = requests.utils.quote(search_query)
        
        # The 'last_sold_date' filter is the key to true verification
        ebay_url = f"https://api.ebay.com/buy/browse/v1/item_summary/search?q={query_encoded}&filter=last_sold_date:[{lookback_date}..]&limit=5"
        ...
        if not items:
            # Fallback to general 'sold' keyword if date filter is too strict for rare cards
...
st.title("TraidLive")
st.markdown("##### AI-Powered Market Verification")
        ...
        if 'suggestions' in st.session_state:
            ...
            for i, crop in enumerate(asset_crops):
                with grid[i % 4]:
                    ...
                    # VERIFICATION BUTTON
                    if st.button(f"Verify Market {i+1}", key=f"v_{i}"):
                        ...
                        <div class="verify-card">
                            ...
                            <p class="listing-title">{l_psa[0]['title'][:60] if l_psa else 'No Match Found'}...</p>
                            <a class="link-text" href="{l_psa[0]['url'] if l_psa else '#'}" target="_blank">🔗 View Sold Item</a>
                            ...
                            <p class="listing-title">{l_raw[0]['title'][:60] if l_raw else 'No Match Found'}...</p>
                            <a class="link-text" href="{l_raw[0]['url'] if l_raw else '#'}" target="_blank">🔗 View Sold Item</a>
                        </div>
                        """, unsafe_allow_html=True)
