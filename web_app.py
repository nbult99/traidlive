...
st.markdown("""
    ...
    .data-label { color: #8E8E93; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
    .link-text { color: #0A84FF; text-decoration: none; font-size: 13px; font-weight: 500; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. THE "MARKET INSIGHTS" ENGINE (FIXED) ---

def fetch_market_valuation(card_name, grade_filter=""):
    """
    Uses the 'last_sold_date' filter to target completed listings.
    Returns: (Avg Price, Data Count, Verification Links)
    """
    ...
    try:
        ...
        # 2. Date Filter (Last 90 days for fresh data)
        ninety_days_ago = (datetime.now() - timedelta(days=90)).strftime('%Y-%m-%dT%H:%M:%S.000Z')
        
        # 3. Targeted Search (Using the query + 'sold' logic)
        search_query = f"{card_name} {grade_filter} sold"
        query_encoded = requests.utils.quote(search_query)
        
        # filter=last_sold_date:[2024-11-20T...] is the secret sauce for transparency
        ebay_url = f"https://api.ebay.com/buy/browse/v1/item_summary/search?q={query_encoded}&filter=last_sold_date:[{ninety_days_ago}..]&limit=5"
...
st.title("TraidLive")
st.markdown("##### Portfolio Market Insights")

# KPI Summary Toolbar
k1, k2, k3 = st.columns(3)
k1.metric("Sync Status", "Cloud Live")
k2.metric("Market Data", "Sold Index")
k3.metric("Verification", "Full History")
                ...
                with grid[i % 4]:
                    ...
                    if st.button(f"Verify Market {i+1}", key=f"v_{i}"):
                        ...
                        <div class="verify-card">
                            ...
                            <a class="link-text" href="{links_psa[0]['url'] if links_psa else '#'}" target="_blank">🔗 View Latest Sold</a>
                            ...
                            <a class="link-text" href="{links_raw[0]['url'] if links_raw else '#'}" target="_blank">🔗 View Latest Sold</a>
                        </div>
                        """, unsafe_allow_html=True)
